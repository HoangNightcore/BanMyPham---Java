...
public class DlgTimMaGiam extends JDialog {
    ...
    public static Product_Sale maGiamTimDuoc = null;
    private Product_SaleBUS giamGiaBUS = new Product_SaleBUS();
    ...
    private void addControls() {
        ...
        columnModelBanHang.getColumn(3).setPreferredWidth(50);
...
//        tblMaGiam.getColumnModel().getColumn(3).setCellRenderer(rightRenderer);
    ...
    private void xuLyChonMaGiam() {
        ...
        if (row > -1) {
            try {
                ...
                String id = tblMaGiam.getValueAt(row, 0) + "";
                String name = tblMaGiam.getValueAt(row, 1) + "";
                ...
                maGiamTimDuoc = new Product_Sale();
                maGiamTimDuoc.setSaleID(id);
                maGiamTimDuoc.setSaleName(name);
                maGiamTimDuoc.setDiscountPercentage(phanTram);
                maGiamTimDuoc.setCondition(dieuKien);
                maGiamTimDuoc.setStartDate(ngayBD);
                maGiamTimDuoc.setEndDate(ngayKT);
    ...
    private void loadDataLenTable() {
        ...
        giamGiaBUS.readSaleList();
        ArrayList<Product_Sale> dsg = giamGiaBUS.getSaleList();
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        DecimalFormat dcf = new DecimalFormat(">###,###");
        for (Product_Sale gg : dsg) {
            Vector vec = new Vector();
            vec.add(gg.getSaleID());
            vec.add(gg.getSaleName());
            vec.add(gg.getDiscountPercentage());
            vec.add(dcf.format(gg.getCondition()));
            vec.add(sdf.format(gg.getStartDate()));
            vec.add(sdf.format(gg.getEndDate()));

            Date now = new Date();
            if (gg.getStartDate().before(now) && gg.getEndDate().after(now)) {
                vec.add("Có hiệu lực");
            } else {
