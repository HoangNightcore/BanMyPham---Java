...
import BanMyPham.BUS.Order_StatusBUS;
import BanMyPham.BUS.OrdersBUS;
import BanMyPham.DTO.Users;
import BanMyPham.GUI.Dialog.DlgTimKhach;
import BanMyPham.GUI.Dialog.DlgTimMaGiam;
...
public class XuatHoaDonGUI extends JDialog {
    ...
    private OrdersBUS hoadonBUS = new OrdersBUS();
    private Order_DetailsBUS ctHoaDonBUS = new Order_DetailsBUS();
    private DlgTimKhach timKhachUI = new DlgTimKhach();
    private DlgTimMaGiam timMaUI;
    private Order_StatusBUS ordStatusBUS = new Order_StatusBUS();
    ...
    public XuatHoaDonGUI(ArrayList<Vector> dsGioHang, int tongTien, Users user) {
        this();
        this.tongTien = tongTien;
        this.dsGioHang = dsGioHang;
        this.user = user;
        this.nhanVien = user.getUsername();
    ...
    private void xuLyHienThiHoaDon() {
        ...
        hd += "Nhân viên: " + user.getFirstName() + " " + user.getLastName() + "<br/>";
        hd += "Ngày lập: " + dtf.format(now) + "<br/>";
        hd += "Khách hàng: " + txtCustomerName.getText() + "<br/>";
        ...
        if (timMaUI.maGiamTimDuoc != null) {
            double percent = 0;
            // lấy phần trăm giảm
            percent = timMaUI.maGiamTimDuoc.getDiscountPercentage();
            if (tongTien >= timMaUI.maGiamTimDuoc.getCondition()) {
        ...
        if (timMaUI.maGiamTimDuoc != null) {
            ...
            hd += "<td style='text-align:center;'>" + timMaUI.maGiamTimDuoc.getDiscountPercentage() + "%" + "</td>";
        }
        hd += "</tr>";
        hd += "<tr>";
        hd += "<td style='text-align:center;'>" + "</td>";
    ...
    private void initComponents() {
        ...
        txtSaleID.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
...
//        }
        ...
        String orderStatusID = ordStatusBUS.getOrdStatusByName("Completed");
        hoadonBUS.saveOrder(user.getUserID(), DlgTimKhach.khachHangTimDuoc.getCustomerID(), orderStatusID, tongTien);
        for (Vector vec : dsGioHang) {
            String maSP = vec.get(0) + "";
            int soLuong = Integer.parseInt(vec.get(2) + "");
            ctHoaDonBUS.addOrderDetail(maSP, soLuong);
        }
        btnThanhToan.setEnabled(false);
    ...
    private void btnTimKhachActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (timKhachUI.khachHangTimDuoc != null) {
            txtCustomerName.setText(timKhachUI.khachHangTimDuoc.getCustomerID() + " - " + timKhachUI.khachHangTimDuoc.getFirstName() + " " + timKhachUI.khachHangTimDuoc.getLastName());
        }
    }
    ...
    private void btnTimMaGiamActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (timMaUI.maGiamTimDuoc != null) {
            txtSaleID.setText(timMaUI.maGiamTimDuoc.getSaleID() + " - " + timMaUI.maGiamTimDuoc.getSaleName());
        }
    }
